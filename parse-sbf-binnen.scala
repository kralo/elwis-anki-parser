...
val mediadir = "./img/" //last slash mandatory
...
//      case "br" => new scala.xml.Text("") //empty dummy
  ...
  for (frage <- content \\ "div" \ "ol") {
    ...
    val antwortencsv = frageoptionen.map(_.text.toString).reduceLeft((x,y)=> x + trenner +y) // alle Frageoptionen zusammen gefaltet
    ...
    println( dreistellig(fragenummer) + trenner + "(" + dreistellig(fragenummer) + ") " + fragentext + trenner + antwortencsv + trenner + Tags)
  }
